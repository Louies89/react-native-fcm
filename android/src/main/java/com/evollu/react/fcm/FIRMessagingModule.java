...
import com.google.android.gms.tasks.OnCompleteListener; //Added by chandrajyoti
import com.google.android.gms.tasks.Task; //Added by chandrajyoti
import android.support.annotation.NonNull; //Added by chandrajyoti
...
public class FIRMessagingModule extends ReactContextBaseJavaModule implements LifecycleEventListener, ActivityEventListener {
    ...
    public void presentLocalNotification(ReadableMap details) {
        Log.d("MessagingService", "Local Notification getting called");
    ...
    @ReactMethod //This Entire method is added by chandrajyoti as per referance from https://github.com/firebase/quickstart-android/blob/931a511fa6ff758f7b20aa1d8935d37ec62e669d/messaging/app/src/main/java/com/google/firebase/quickstart/fcm/java/MainActivity.java
    public void subscribeToTopic1(String topic, final Promise promise){
	    try {
    ...
    private void registerTokenRefreshHandler() {
        // IntentFilter intentFilter = new IntentFilter("com.evollu.react.fcm.FCMRefreshToken"); //Commented By Chandrajyoti
        IntentFilter intentFilter = new IntentFilter("com.evollu.react.fcm.FCMRefreshTokenCleNet"); //FCMRefreshToken changed to FCMRefreshTokenCleNet, so that the broadcast shall be only to CleNet even if some other app uses this fcm library
	...
	@ReactMethod
	public void cancelService(boolean cancelOnlyForeGrnd,int notifTm) { //Added by Chandrajyoti
		if(cancelOnlyForeGrnd){
			// Log.d("MessagingService", "cancelService --------- 1");
			Intent serviceIntent = new Intent(getReactApplicationContext(), NotificationForeGroundServiceHJS.class);
			serviceIntent.putExtra("stop",true);
			serviceIntent.putExtra("notifTm",notifTm);
			ContextCompat.startForegroundService(getReactApplicationContext(), serviceIntent);
			// Log.d("MessagingService", "cancelService --------- 2");
		}
		else{
			// Log.d("MessagingService", "stopService --------- 3");
			Intent serviceIntent = new Intent(getReactApplicationContext(), NotificationForeGroundServiceHJS.class);
			getReactApplicationContext().stopService(serviceIntent);
			// Log.d("MessagingService", "stopService --------- 4");
		}
    ...
    private void registerMessageHandler() {
        // IntentFilter intentFilter = new IntentFilter("com.evollu.react.fcm.ReceiveNotification"); //Commented By Chandrajyoti
        IntentFilter intentFilter = new IntentFilter("com.evollu.react.fcm.ReceiveNotificationCleNet"); //ReceiveNotification changed to ReceiveNotificationCleNet, so that the broadcast shall be only to CleNet even if some other app uses this fcm library
    ...
    private void registerLocalMessageHandler() {
        ...
        LocalBroadcastManager.getInstance(getReactApplicationContext()).registerReceiver(new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
				// Log.v(TAG, "registerLocalMessageHandler called");

				// Bundle bundle = intent.getExtras();  //Added below for experiment by chandrajyoti
				// for (String key : bundle.keySet())
				// {
				// 	Log.d("MessagingService", key + " = \"" + bundle.get(key) + "\"");
				// }
