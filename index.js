...
FCM.subscribeToTopic = (topic)  => new Promise ((resolve,reject)=>{
  RNFIRMessaging.subscribeToTopic(topic).then((rslt)=>{
    resolve(rslt);
  })
  .catch((e)=>{
    reject(e);
...
FCM.subscribeToTopic1 = (topic) => new Promise ((resolve,reject)=>{ //Added by chandrajyoti
  RNFIRMessaging.subscribeToTopic1(topic).then((rslt)=>{
    resolve(rslt);
  })
  .catch((e)=>{
    resolve(e);
...
FCM.cancelService = (cancelOnlyForeGrnd=false,notifTm = 0) => { //Added by chandrajyoti
  // cancelOnlyForeGrnd = false; //false:completely close the service, true:only foreground notification shall be cancelled but service shal go on
  // notifTm = <notification Start time>
  if (Platform.OS !== 'ios') {
    RNFIRMessaging.cancelService(cancelOnlyForeGrnd,notifTm);
  }
}
